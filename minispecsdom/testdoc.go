...
func (t *TestDoc) parse(src string) {
	t.parser = schema.NewMarkdownParser()
	t.doc = sdom.Parse(src, 0, t.parser)
	t.ctx = t.parser.Indent().Brackets().Context()
	t.scan()
}
...
func (t *TestDoc) Doc() *sdom.Doc          { return t.doc }
func (t *TestDoc) Render() (string, error) { return t.doc.Render() }
...
// Seq: seq-testdoc.md#1.3 — a region ends at the next heading of level 2 or higher.
func (t *TestDoc) regionEnd(start int) int {
	nodes := t.doc.Nodes()
...
func (t *TestDoc) readFields(e *TestEntry, run []sdom.Node) {
	...
	for i, l := range strings.SplitAfter(b.String(), "\n") {
		...
		if m != nil && !t.inCode(run, lineStart) {
...
func (t *TestDoc) inCode(run []sdom.Node, off int) bool {
	n := nodeAt(run, off)
	if n == nil {
		return false
	}
	lang := t.ctx.Language()
	for enc := t.ctx.Enclosing(n); enc != nil; enc = t.ctx.Enclosing(enc) {
		s, _ := enc.Render()
...
func (t *TestDoc) replaceSpan(start, end int, text string) error {
	right, err := t.boundary(end)
	if err != nil {
		return err
	}
	left, err := t.boundary(start)
	if err != nil {
		return err
	}
	var gone []sdom.Node
	inside := false
	for _, n := range t.doc.Nodes() {
		if n == left {
			inside = true
		}
		if n == right {
	...
	for _, n := range gone {
		if err := t.doc.Remove(n); err != nil {
			return err
		}
	}
	return t.doc.Insert(right, sdom.NewText(text, sdom.Synthetic(len(text))))
}
...
func (t *TestDoc) boundary(off int) (sdom.Node, error) {
	nodes := t.doc.Nodes()
	n := nodeAt(nodes, off)
	if n == nil {
		if off >= t.total {
	...
	_, right, err := t.doc.Split(n, off-n.Location().Offset())
	return right, err
}
