...
- [ ] O29: minispecsdom/unread.go — the helper the seven readers share for the context's unclosed, unpaired and demoted lists — is mapped by no Artifacts line in design.md, so its Rn refs (R300–R303, R311, R351) harvest through no artifact and a future anchor there is unseen; map it under crc-Done.md or give it a card
- [ ] O30: Gaps.Add appends its line to a file with no final newline without supplying one, so the new gap glues onto the last gap's line and the read-back finds nothing (measured 2026-09-07 on this design.md at ceafd0c: every add-gap refused until a newline was added by hand). The write should end the previous line before appending, as Requirements.Add and the trajectory writers do; the same check belongs on every append-at-end path
