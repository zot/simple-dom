...
- [x] O28: Pending.Place accepts entry text that opens a bracket group never closed (a lone asterisk in a status, measured 2026-09-06 on this queue): the entry itself reads back, so the check passes, and every later entry falls inside the unclosed run and is not read. A write path should refuse text whose placement grows Unread, since the file it produces is one its own reader cannot fully read. Retired 2026-09-16: minispecsdom left this tree at 152a5b9 (see carves/done/minispecsdom-move.md in mini-spec); the gap continues there as O25.
- [x] O29: minispecsdom/unread.go — the helper the seven readers share for the context's unclosed, unpaired and demoted lists — is mapped by no Artifacts line in design.md, so its Rn refs (R300–R303, R311, R351) harvest through no artifact and a future anchor there is unseen; map it under crc-Done.md or give it a card Retired 2026-09-16: minispecsdom left this tree at 152a5b9 (see carves/done/minispecsdom-move.md in mini-spec); closed there by the Artifacts mapping of the port.
- [x] O30: Gaps.Add appends its line to a file with no final newline without supplying one, so the new gap glues onto the last gap's line and the read-back finds nothing (measured 2026-09-07 on this design.md at ceafd0c: every add-gap refused until a newline was added by hand). The write should end the previous line before appending, as Requirements.Add and the trajectory writers do; the same check belongs on every append-at-end path Retired 2026-09-16: minispecsdom left this tree at 152a5b9 (see carves/done/minispecsdom-move.md in mini-spec); the gap continues there as O26.
